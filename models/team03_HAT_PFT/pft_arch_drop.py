...
class PFTransformerLayer(nn.Module):
    ...
    def forward(self, x, pfa_list, x_size, params):
        ...
        x = shortcut + self.drop_path(x_win.view(b, n, c))
        x = x + self.drop_path(self.convffn(self.norm2(x), x_size))
    ...
    def forward_inference(self, x, pfa_list, x_size, params):
        ...
        x.add_(self.drop_path(attn_x.view(b, n, c)))
        del attn_x

        x_norm2 = self.norm2(x)
        x_ffn = self.convffn(x_norm2, x_size)
        del x_norm2
        x.add_(self.drop_path(x_ffn))
        del x_ffn
    ...
    def __init__(
        ...
        for i in range(self.depth):
            if isinstance(drop_path, (list, tuple)):
                dpr = float(drop_path[int(layer_id) + i])
            else:
                dpr = float(drop_path) if drop_path is not None else 0.0
            self.layers.append(
                PFTransformerLayer(
                    dim=self.dim,
                    block_id=int(idx),
...
class PFT(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        dpr = [x.item() for x in torch.linspace(0, float(drop_path_rate), total_layers)]
